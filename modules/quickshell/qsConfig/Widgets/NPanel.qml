...
Loader {
  ...
  // properties to allow positioning relative to opener
  ...
  function open(buttonItem) {
    // Get button position if one is provided
    ...
    // special case if the panel is currently closing
    if (isClosing) {
      hideTimer.stop()
      scaleValue = 1.0
      opacityValue = 1.0
  ...
  // Timer to disable loader after closing animation is done
  Timer {
    id: hideTimer
  ...
  sourceComponent: Component {
    PanelWindow {
      ...
      visible: true
      color: Qt.alpha(Colors.mShadow, dimmingOpacity)

      WlrLayershell.exclusionMode: ExclusionMode.Ignore
      WlrLayershell.namespace: "custom-ui-panel"
      ...
      Rectangle {
        ...
        property real marginTop: {
          ...
          case "top":
            return (Style.barHeight + Style.marginS) * scaling
          default:
